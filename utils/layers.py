...
class GuideDecoderLayer(nn.Module):
    ...
    def __init__(self, in_channels:int, output_text_len:int, input_text_len:int=24, embed_dim:int=768):
        ...
        self.prompt = nn.Parameter(torch.zeros(output_text_len, in_channels),requires_grad=True)

    def forward(self,x,txt,epoch, feat):
        ...
        q = k = self.vis_pos(vis2)   #(8, 49, 768) (8, 196, 384) (8, 784, 192)
        #######
        if epoch > 20:
            vis2 = self.norm1(x)
...
class GuideDecoder(nn.Module):
    ...
    def forward(self, vis, skip_vis, txt, epoch, feat):

        if txt is not None:
            vis,retxt =  self.guide_layer(vis, txt, epoch, feat)
